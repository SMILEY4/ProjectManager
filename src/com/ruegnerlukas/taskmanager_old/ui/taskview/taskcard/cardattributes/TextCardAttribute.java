...
public class TextCardAttribute extends CardAttribute {


	protected TextCardAttribute(Task task, TaskAttribute attribute) {
	...
	protected Region getIconNode(TaskAttributeValue value) {
		if (!(value instanceof NoValue)) {
			...
			Label label = new Label(value.getValue().toString().substring(0, Math.min(5, value.getValue().toString().length())));
