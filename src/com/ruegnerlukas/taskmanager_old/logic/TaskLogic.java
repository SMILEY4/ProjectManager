...
public class TaskLogic {
	...
	TaskLogic() {
		...
		EventManager.registerListener(e -> {
			...
			if (isAttributeRelevant(event.getAttribute()) || com.ruegnerlukas.taskmanager.logic.Logic.filter.isAttributeRelevant(event.getAttribute()) || com.ruegnerlukas.taskmanager.logic.Logic.group.isAttributeRelevant(event.getAttribute()) || com.ruegnerlukas.taskmanager.logic.Logic.sort.isAttributeRelevant(event.getAttribute())) {
				EventManager.fireEvent(new RefreshTaskDisplayRecommendationEvent(this));
			}
	...
	protected List<Task> getTasksInternal() {
		return com.ruegnerlukas.taskmanager.logic.Logic.project.getProject().tasks;
	}
...
//		}
	...
	private void onAttributeChanged(TaskAttribute attribute, Map<TaskAttributeData.Var, TaskAttributeValue> changedVars) {
		...
		if (changedVars.containsKey(TaskAttributeData.Var.FLAG_ATT_FLAGS)) {
			FlagAttributeData flagData = (FlagAttributeData) com.ruegnerlukas.taskmanager.logic.Logic.attribute.findAttribute(TaskAttributeType.FLAG).data;
			List<Task> taskList = com.ruegnerlukas.taskmanager.logic.Logic.project.getProject().tasks;
			for (Task task : taskList) {
				TaskFlag currFlag = ((FlagValue) getValue(task, com.ruegnerlukas.taskmanager.logic.Logic.attribute.findAttribute(FlagAttributeData.NAME))).getFlag();
	...
	public Response<TaskGroupData> getTaskGroups() {
		...
		if (project != null) {
			List<Task> allTasks = project.tasks;
			List<Task> filteredTasks = com.ruegnerlukas.taskmanager.logic.Logic.filter.applyFilters(allTasks);
			TaskGroupData groupedTasks = com.ruegnerlukas.taskmanager.logic.Logic.group.applyGroups(filteredTasks);
			com.ruegnerlukas.taskmanager.logic.Logic.sort.applySort(groupedTasks);
	...
	public Response<TaskAttributeValue> getAttributeValue(Task task, String attributeName) {
		...
		if (project != null) {
			...
			if (task == null) {
				...
				TaskAttribute attribute = com.ruegnerlukas.taskmanager.logic.Logic.attribute.findAttribute(attributeName);
				if (attribute == null) {
					return new Response<TaskAttributeValue>().complete(null, Response.State.FAIL);
	...
	public Response<Task> createNewTask() {
		...
		if (project != null) {
			...
			setValue(task, com.ruegnerlukas.taskmanager.logic.Logic.attribute.findAttribute(TaskAttributeType.ID), new NumberValue(project.idCounter++));
	...
	public void deleteTask(Task task) {
		...
		if (project != null) {
			...
			if (removed) {
				...
				List<TaskAttribute> attributes = com.ruegnerlukas.taskmanager.logic.Logic.attribute.findAttributes(TaskAttributeType.DEPENDENCY);
				for (TaskAttribute attribute : attributes) {
					List<Task> dependOn = com.ruegnerlukas.taskmanager.logic.Logic.dependencies.getDependentOnInternal(task, attribute);
					for (Task dep : dependOn) {
						com.ruegnerlukas.taskmanager.logic.Logic.dependencies.deleteDependency(dep, task, attribute);
					}
				}
