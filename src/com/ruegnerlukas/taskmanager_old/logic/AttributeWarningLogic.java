...
public class AttributeWarningLogic {
	...
	private boolean requiresWarningTasks(TaskAttribute attribute, Map<TaskAttributeData.Var, TaskAttributeValue> newValues) {
		...
		AttributeValidator validator = com.ruegnerlukas.taskmanager.logic.Logic.attribute.getAttributeValidator(attribute.data.getType());
		AttributeUpdater updater = com.ruegnerlukas.taskmanager.logic.Logic.attribute.getAttributeUpdater(attribute.data.getType());
		...
		for (int i = 0, n = com.ruegnerlukas.taskmanager.logic.Logic.tasks.getTasksInternal().size(); i < n; i++) {
			Task task = com.ruegnerlukas.taskmanager.logic.Logic.tasks.getTasksInternal().get(i);
