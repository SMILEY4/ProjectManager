	...
	public void update(List<Node> outNodes, TaskAttributeData data, FilterCriteria.ComparisonOp compOp, Object compValue) {
		...
		if (FilterCriteria.ComparisonOp.EQUALITY == compOp || FilterCriteria.ComparisonOp.INEQUALITY == compOp) {
			...
			if(numberData.decPlaces > 0) {
				value = 0.0;
				if(compValue instanceof Double) {
					value = compValue;
				}
				Spinner<Double> spinner = buildDoubleSpinner(min, max, 1.0/Math.pow(10,decPlaces), decPlaces, (Double)value);
				...
				Spinner<Integer> spinner = buildIntSpinner((int)min, (int)max, 1, (Integer)value);
				outNodes.add(spinner);
				spinner.valueProperty().addListener((observable, oldValue, newValue) -> {
					value = spinner.getValue();
					onAction();
		...
		if (FilterCriteria.ComparisonOp.IN_RANGE == compOp || FilterCriteria.ComparisonOp.NOT_IN_RANGE == compOp) {
			...
			if(numberData.decPlaces > 0) {
				value = new Double[]{0.0, 999.0};
				if(compValue instanceof Double[] && ((Double[])compValue).length == 2) {
					value = compValue;
				}

				Spinner<Double> spinnerMin = buildDoubleSpinner(min, max, 1.0/Math.pow(10,decPlaces), decPlaces, ((Double[])value)[0] );
				outNodes.add(spinnerMin);
				spinnerMin.valueProperty().addListener((observable, oldValue, newValue) -> {
					value = new Double[]{ spinnerMin.getValue(), ((Double[])value)[1]};
					onAction();
				});

				Spinner<Double> spinnerMax = buildDoubleSpinner(min, max, 1.0/Math.pow(10,decPlaces), decPlaces, ((Double[])value)[1] );
				outNodes.add(spinnerMax);
				spinnerMax.valueProperty().addListener((observable, oldValue, newValue) -> {
					value = new Double[]{ ((Double[])value)[0], spinnerMax.getValue()};
					onAction();
				});
				...
				Spinner<Integer> spinnerMin = buildIntSpinner((int)min, (int)max, 1, ((Integer[])value)[0] );
				outNodes.add(spinnerMin);
				spinnerMin.valueProperty().addListener((observable, oldValue, newValue) -> {
				...
				Spinner<Integer> spinnerMax = buildIntSpinner((int)min, (int)max, 1, ((Integer[])value)[1] );
				outNodes.add(spinnerMax);
				spinnerMax.valueProperty().addListener((observable, oldValue, newValue) -> {
